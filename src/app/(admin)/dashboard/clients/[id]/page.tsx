...
export default function ClientDetailPage() {
  ...
  const loadStats = async () => {
    try {
      ...
      const [
        ...
        fetch(`/api/photos?clientId=${clientId}`),
      ...
      const [documents, designProjects, estimates, news, schedule, photos, receipts] = await Promise.all([
        ...
        photosRes.ok ? photosRes.json() : { photoBlocks: [] },
        receiptsRes.ok ? receiptsRes.json() : { receiptBlocks: [] }
      ]);
      ...
      setStats({
        ...
        estimates: {
          ...
          visible: estimates.estimates?.filter((e: any) => e.showToClient).length || 0,
          draft: estimates.estimates?.filter((e: any) => !e.showToClient).length || 0
        },
        news: {
          total: news.news?.length || 0,
          recent: news.news?.filter((n: any) => 
        ...
        schedule: {
          ...
          activeTasks: schedule.projects?.reduce((acc: number, p: any) => 
            acc + p.tasks.filter((t: any) => t.status === 'in_progress').length, 0
          ) || 0
        },
        photos: {
          total: photos.photoBlocks?.reduce((acc: number, block: any) => 
            acc + block.photos.length, 0
          ) || 0,
  ...
  const menuItems = [
    ...
    },
    {
      ...
      icon: Calendar,
      href: `/dashboard/clients/${clientId}/schedule`,
    ...
    },
    {
      ...
      icon: Newspaper,
      href: `/dashboard/clients/${clientId}/news`,
    ...
    },
    {
      ...
      icon: Video,
      href: `/dashboard/clients/${clientId}/video`,

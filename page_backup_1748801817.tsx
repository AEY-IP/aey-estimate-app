...
export default function EditEstimatePage({ params }: { params: { id: string } }) {
  ...
  const saveEstimate = async () => {
    ...
    try {
      ...
      // Только для смет по квартире обрабатываем worksBlock и materialsBlock
      if (estimate.type === 'apartment' && estimate.worksBlock) {
        // Пересчитываем суммы блоков работ
        const updatedBlocks = estimate.worksBlock.blocks.map(block => ({
          ...block,
          totalPrice: block.items.reduce((sum, item) => sum + item.totalPrice, 0)
        }))
        
        // Получаем ручные коэффициенты для сохранения в смете
        const manualCoeffsForEstimate = coefficients.filter(c => c.id.startsWith('manual_'))
        
        // Пересчитываем итоговые суммы перед сохранением
        const updatedEstimate = {
          ...estimate,
          totalWorksPrice,
          totalMaterialsPrice,
          totalPrice: grandTotal,
          manualCoefficients: manualCoeffsForEstimate, // Сохраняем ручные коэффициенты в смете
          coefficientSettings, // Сохраняем настройки применения коэффициентов
          manualPrices: Array.from(manuallyEditedPrices), // Сохраняем ID позиций с ручной ценой
          ...
          } : undefined, // Сохраняем параметры помещения
          worksBlock: {
            ...estimate.worksBlock,
        ...
        // Для смет по помещениям - временная заглушка
        alert('Сохранение смет по помещениям пока не реализовано')
      }
    } catch (error) {
  ...
  const updateWorkQuantitiesForParameter = (parameterId: string, value: number) => {
    ...
    if (estimate.type !== 'apartment' || !estimate.worksBlock) {
      console.log('Обновление количества доступно только для смет по квартире')
      return
    }
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="container mx-auto px-6 py-8">
        <div className={`grid gap-8 ${
          // Показываем коэффициенты только для смет по квартире или сводной сметы помещений
          ...
          {/* Правая колонка - коэффициенты (только для смет по квартире или сводной сметы) */}
          {(estimate?.type === 'apartment' || (estimate?.type === 'rooms' && isSummaryView)) && (
